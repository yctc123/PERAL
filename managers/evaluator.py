    ...
    def eval(self, save=False):
        ...
        with torch.no_grad():
            for b_idx, batch in enumerate(dataloader):
                ...
                pos_scores += score_pos.view(-1).detach().cpu().tolist()
                neg_scores += score_neg.view(-1).detach().cpu().tolist()
                pos_labels += targets_pos.view(-1).tolist()
                neg_labels += targets_neg.view(-1).tolist()
