...
def sample_neg(adj_list, edges, num_neg_samples_per_link=1, max_size=1000000, constrained_neg_prob=0):
    ...
    if max_size < len(pos_edges):
        perm = rng.permutation(len(pos_edges))[:max_size]
    ...
    while len(neg_edges) < num_neg_samples_per_link * len(pos_edges):          
        ...
        if rng.uniform() < constrained_neg_prob:
            if rng.uniform() < 0.5:
                neg_head = rng.choice(valid_heads[rel])
            else:
                neg_tail = rng.choice(valid_tails[rel])
        else:
            if rng.uniform() < 0.5:
                neg_head = rng.choice(n)
            else:
                neg_tail = rng.choice(n)
...
def links2subgraphs(A, graphs, params, max_label_value=None):
    ...
    for split_name, split in graphs.items():
        pos_count = len(split['pos'])
        neg_count = len(split['neg'])
        total_subgraphs += pos_count + neg_count

        total_subgraphs += pos_count * 2

    map_size = total_subgraphs * BYTES_PER_DATUM
    map_size = int(map_size)
    logging.info(f"Total subgraphs to store: {total_subgraphs}")
    logging.info(f"Estimated map_size: {map_size / (1024**3):.2f} GB")
    env = lmdb.open(params.db_path, map_size=map_size, max_dbs=20)


    def extraction_helper(A, links, g_labels, split_env ,extract_func):
        ...
        with mp.Pool(processes=None, initializer=intialize_worker, initargs=(A, params, max_label_value)) as p:
            ...
            for (str_id, datum) in tqdm(p.imap(extract_func, args_), total=len(links)):
    ...
    for split_name, split in graphs.items():
        ...
        extraction_helper(A, split['pos'], labels, split_env,extract_save_subgraph)
        ...
        extraction_helper(A, split['neg'], labels, split_env,extract_save_subgraph)


        logging.info(f"Extracting contrastive subgraphs (view1) for positive links in {split_name} set")
        labels = np.ones(len(split['pos']))
        db_name_cont1 = split_name + '_pos_contrast1'
        split_env1 = env.open_db(db_name_cont1.encode())
        extraction_helper(A, split['pos'], labels, split_env1, extract_save_subgraph_contrast_cl)

        logging.info(f"Extracting contrastive subgraphs (view2) for positive links in {split_name} set")
        db_name_cont2 = split_name + '_pos_contrast2'
        split_env2 = env.open_db(db_name_cont2.encode())
        extraction_helper(A, split['pos'], labels, split_env2, extract_save_subgraph_contrast_cl)
...
def extract_save_subgraph_contrast_cl(args_):
    'Helper function.'
    idx, (n1, n2, r_label), g_label = args_
    params_.enclosing_sub_graph = False
    nodes, n_labels, subgraph_size, enc_ratio, num_pruned_nodes = subgraph_extraction_labeling_cl((n1, n2), r_label, A_, params_.hop, params_.enclosing_sub_graph, params_.max_nodes_per_hop)
...
def subgraph_extraction_labeling_cl(ind, rel, A_list, h=1, enclosing_sub_graph=False, max_nodes_per_hop=None, max_node_label_value=None):
    A_incidence = incidence_matrix(A_list)
    A_incidence += A_incidence.T
    ...
    if enclosing_sub_graph:
        ...
        diff_nodes = list(subgraph_nei_nodes_un - subgraph_nei_nodes_int)
        half = (len(diff_nodes) + 1) // 2
        selected_diff = random.sample(diff_nodes, half) if diff_nodes else []
        subgraph_nodes = list(ind) + list(subgraph_nei_nodes_int) + selected_diff
