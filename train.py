...
from utils.graph_utils import collate_dgl, collate_dgl_train, move_batch_to_device_dgl, move_batch_to_device_dgl_train,collate_dgl_train_cor,move_batch_to_device_dgl_train_cor
...
def main(params):
    ...
    train = SubgraphDataset(params.db_path, 'train_pos', 'train_neg', params.file_paths,params.max_paths,
                            ...
                            use_llm_path_filter=params.use_llm_path_filter,
                            llm_path_desc_dir=params.llm_path_desc_dir,
                            llm_path_cache=params.llm_path_cache,
                            llm_path_model=params.llm_path_model,
                            llm_path_backend=params.llm_path_backend,
                            llm_path_local_model=params.llm_path_local_model,
                            llm_path_device_map=params.llm_path_device_map,
                            llm_path_torch_dtype=params.llm_path_torch_dtype,
                            llm_path_max_candidates=params.llm_path_max_candidates)
    valid = SubgraphDataset(params.db_path, 'valid_pos', 'valid_neg', params.file_paths,params.max_paths,
                            add_traspose_rels=params.add_traspose_rels,
                            num_neg_samples_per_link=params.num_neg_samples_per_link,
                            use_kge_embeddings=params.use_kge_embeddings, dataset=params.dataset,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--num_epochs", "-ne", type=int, default=35,
    ...
    parser.add_argument('--coef_dgi_loss', type=float, default=0.1, help='Coefficient of MI loss')
    ...
    parser.add_argument('--use_llm_path_filter', action='store_true',
                        help='use an LLM to rank 2-hop and 3-hop candidate paths; 1-hop paths are kept directly')
    parser.add_argument('--no_llm_path_filter', action='store_true',
                        help='disable LLM path ranking and keep graph-extracted candidate path order')
    parser.add_argument('--llm_path_desc_dir', type=str, default=None,
                        help='directory containing entity2text.txt and relation2text.txt for LLM path ranking')
    parser.add_argument('--llm_path_cache', type=str, default=None,
                        help='jsonl cache file for LLM path ranking results')
    parser.add_argument('--llm_path_model', type=str, default='gpt-4o-mini',
                        help='OpenAI model used for LLM path ranking')
    parser.add_argument('--llm_path_backend', type=str, default='openai', choices=['openai', 'local_qwen', 'qwen', 'transformers'],
                        help='LLM backend for path ranking')
    parser.add_argument('--llm_path_local_model', type=str, default=None,
                        help='local model directory for Qwen path ranking')
    parser.add_argument('--llm_path_device_map', type=str, default='auto',
                        help='device_map passed to transformers.from_pretrained for local Qwen')
    parser.add_argument('--llm_path_torch_dtype', type=str, default='auto', choices=['auto', 'float16', 'fp16', 'bfloat16', 'bf16', 'float32', 'fp32'],
                        help='torch dtype for loading local Qwen')
    parser.add_argument('--llm_path_max_candidates', type=int, default=50,
                        help='maximum number of 2-hop/3-hop candidate paths collected before LLM ranking; set 0 to disable the cap')
    ...
    params.collate_fn_train = collate_dgl_train_cor
    params.move_batch_to_device_train = move_batch_to_device_dgl_train_cor
    main(params)
