...
def process_files(files, saved_relation2id=None, add_traspose_rels=False, sort_data=False):
    ...
    csc_matrix(
        (
            np.ones(len(idx), dtype=np.uint8),                     # data
            (triplets['train'][:, 0][idx].squeeze(1),              # row indices (heads)
             triplets['train'][:, 1][idx].squeeze(1))              # col indices (tails)
        ),
        shape=(len(entity2id), len(entity2id))                    # matrix shape
    )
)
    '''
